...
export class AuthService {
  ...
  constructor(private storageService: StorageService) {
    this.addRole(this.client);
    this.addRole(this.admin);
    this.addRole(this.bankWorker);
  }
  ...
  loginUser(user: IUser) {
    ...
    if (user.roles.includes('client')) {
      this.redirectUrl = '/api/v1/banks/bankList/';
    } else if (user.roles.includes('admin')) {
      this.redirectUrl = '/accounts';
    } else if (user.roles.includes('bankWorker')) {
      this.redirectUrl = '/api/v1/bankWorker/';
    } 
